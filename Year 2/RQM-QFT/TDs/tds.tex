...
\begin{enumerate}
\item Let $\vb{A}$ and $\vb{B}$ in $\mathcal{C}_+$. Then $\vb{A} \cdot \vb{B} < 0$. 
...
\item Suppose by induction that this is true for $n$ the base cases being trivial. Then for $n+1$ note that $C_+$ is stable under addition so any case can be reduced to the base case $n = 2$. We prove this case here:
\[
\sqrt{-(\vb{A} + \vb{B})^2} = \sqrt{-(\vb{A'} + \vb{B'})^2} = \sqrt{{a^0}^2 + 2 a^0 b^0 + {b^0}^2 - \vec{a}^2} \geq {a_0} + b_0 - || \vec{a} || \geq \sqrt{-\vb{A}^2} + \sqrt{-\vb{B}^2}
\]
\end{enumerate}
